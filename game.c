...
int main(int argc, char **argv)
{
	...
	add_neighboring_room(bedroom, bathroom);
	add_neighboring_room(bathroom, bedroom);
	add_neighboring_room(bathroom, living_room);
	add_neighboring_room(living_room, bathroom);
	add_neighboring_room(living_room, kitchen);
	add_neighboring_room(living_room, outside);
	add_neighboring_room(kitchen, living_room);

	// Stash all the rooms into an array for easier management later.
	Room *rooms[] = { bedroom, bathroom, living_room, kitchen, outside };

	// Items found in the bedroom.
	Item *clothes;
	construct_item(&clothes, "Clothes", "Put on some clothes", "The clothes are cold, but way better than nothing.", BODY_TEMPERATURE, 50);
	...
	for (;;)
	{
		...
		if (selected_option <= current_room->number_of_items)
		{
			...
			remove_item(current_room, selected_item->name);
		...
		{	
			...
			if (selected_room == outside)
			{
				...
				{
					printf("You're still so tired you'll probably fall and break your neck.\n");
				}
				else
				{
